...
const faculties = [
    ...
    },
    {
      ...
      FacultyName: "aespoke Concrete Sausages",
...
app.get('/faculty/:id',(req,res)=>{
    ...
    const index = faculties.findIndex(temp=>temp.id == id);
...
// print faculties which name starts with A
app.get("/faculty/starts/:startsWith",(req,res)=>{
    const data = faculties.filter((temp)=>{
        if(temp.FacultyName.toLowerCase().startsWith(req.params.startsWith.toLowerCase())){
            return true;
        }
    });
    const ans = {
...
//print faculties who is born before 1980
app.get("/faculty/bornBefore/:year",(req,res)=>{
    const year = req.params.year;
    const data = faculties.filter((temp)=>{
        if(Number(temp.FacultyDateOfBirth.split("-")[0])<year){
            return true;
        }
    });
    const ans = {
