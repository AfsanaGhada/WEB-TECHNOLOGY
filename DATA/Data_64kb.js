...
app.get('/spiData/filtered', (req, res) => {
    const filteredspiData = spiData.filter(spi => spi.spi > 5);
    const ans = {
        isSuccess: true,
        count: filteredspiData.length,
        msg: "Data retrieved successfully",
        data: filteredspiData
    };
    res.send(ans);
});
...
// task 4: print number of records which has less than 5 spi
app.get('/spiData/filtered/lessthan5', (req, res) => {
    const filteredspiData = spiData.filter(spi => spi.spi < 5);
    const ans = {
        isSuccess: true,
        count: filteredspiData.length,
...
// task 5: print all record with Hindi language
app.get('/spiData/filtered/Hindi', (req, res) => {
    const filterhindilan = spiData.filter(hindilag => hindilag.language = "Hindi");
    const answer = {
        isSuccess: true,
        count: filterhindilan.length,
        msg: "Data retrieved successfully",
        data: filterhindilan
    };
    res.send(answer);
...
// task 6: print all record with Sindhi language and name starts with A
app.get('/spiData/filtered/sindhi&name_starts_with_A', (req, res) => {
    const filterSindhilan = spiData.filter(record => record.language = "Sindhi" && record.name.startsWith("A"));
    const answer = {
        isSuccess: true,
        count: filterSindhilan.length,
        msg: " print all record with Sindhi language and name starts with A",
        data: filterSindhilan
    };
    res.send(answer);
});
